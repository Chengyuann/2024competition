...
root_dir = '/home/macy/Data/2024competition/gao/data/训练集/图片'
...
LR = 0.001
num_classes = 5
gpu_id = 2  # 选择你的GPU ID，通常是0, 1, 2, 等
...
for epoch in range(EPOCHS):
    ...
    val_acc_ = 0
    with torch.no_grad():
        for idx, (inputs, labels) in enumerate(Bar(val_loader)):
            inputs, labels = inputs.to(device), labels.to(device)
            outputs = model(inputs)
            predict_y = torch.max(outputs, dim=1)[1]
            loss = criterion(outputs, labels)
    ...
    torch.save(model.state_dict(), './efficientnet_last.pth')
